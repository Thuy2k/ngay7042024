...
@section('content')
    <section class="charts">
        <section>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header" style="display:flex;">
                                <form id="delete-product-detail"
                                    action="{{ route('admin.product_detail.delete', ['id' => $product->id]) }}"
                                ...
                                <div class="card-body" style="padding:0;">
                                    <form class="form-inline" style="float: right;" method="GET"
                                        action="{{ route('admin.product_detail.index', ['id' => $product->id]) }}">
                                        <div class="form-group pr-2">
                                            <select name="search_size" id="search_size" class="form-control">
                                                <option value="">--Chọn kích thước--</option>
                                                @foreach ($sizes as $size)
                                                    <option @if (Request::get('search_size') == $size->id) selected @endif
                                        ...
                                        <div class="form-group pr-2">
                                            <select name="search_color" id="search_color" class="form-control">
                                                ...
                                                @foreach ($colors as $color)
                                                    <option @if (Request::get('search_color') == $color->id) selected @endif
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($rows as $row)
                                                <tr>
                                                    <td><input type="checkbox" name="check" class="check cursorc"
                                                            data-id="{{ $row->id }}"></td>
                                                    ...
                                                    <td>{{ $row->quantity }}</td>
                                                    <td>
                                                        <span class="edit cursor" data-size="{{ $row->size->id }}"
                                                            data-color="{{ $row->color->id }}"
                                                            data-quantity="{{ $row->quantity }}"
                                                            data-id="{{ $row->id }}"><i class="fa fa-edit"></i></span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    ...
                                    <div class="mt-3" style="display: flex;justify-content: center;">
                                        <center>{{ $rows->withQueryString()->links() }}</center>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div id="modal-product-detail" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div role="document" class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="form-product-detail"
                                action="{{ route('admin.product_detail.store', ['id' => $product->id]) }}" method="POST"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        <div class="form-group">
                                            @foreach ($arrPathImageNeed as $image)
                                                <label>
                                                    <input type="radio" name="productImage"
                                                        value="{{ $image }}">
                                                    <img src="{{ asset($image) }}" alt="Ảnh sản phẩm">
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
                      ...
                      <img width="200px" height="200px" id="ImgPre" @if ($flag && !empty($rows->avatar)) src="{{ asset($rows->avatar)}}" @elsesrc="{{ asset('images/none-user.png')}}" alt="Alternate Text" class="img-thumbnail" />
                        ...
                        <div class="modal-footer">
                            <button type="button" data-dismiss="modal"
                                class="btn btn-disabled btn-secondary">Đóng</button>
