                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header" style="display:flex;">
                                ...
                                {{-- Form delete --}}
                                <form id="delete-order" action="{{ route('admin.order.delete') }}" method="POST"
                                    style=" display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="list_id" id="list_id" value="">
                                    <button disabled type="submit"
                                        class="btn btn-danger btn-delete">{{ $title_delete }}</button>
                                </form>
                        ...
                        <input id="date" value="{{Request::get('date')}}" name="date" type="date" class="mr-3 form-control form-control">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input id="inlineFormInputGroup" value="{{ Request::get('search') }}"
                                                    name="search" type="text" placeholder="Nhập mã đơn hàng"
                                        ...
                                        <tbody>
                                            @foreach ($rows as $row)
                                                <tr>
                                                    ...
                                                    <td><button id="btn-paid-{{ $row->id }}"
                                                            class="btn btn-paid {{ $row->payment->status ? 'btn-success' : 'btn-danger' }}"
                                                            data-id="{{ $row->id }}">{{ $row->payment->status ? 'Đã thanh toán' : 'Chưa thanh toán' }}</button>
                                                    </td>
                                                    <td>{{ format_date($row->created_at) }}</td>
                                                    <td>{{ $row->note }}</td>
                                                    <th>
                                                        <span class="info cursor" data-id="{{ $row->id }}"
