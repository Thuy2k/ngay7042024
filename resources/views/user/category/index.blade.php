            ...
            <div class="row">
                ...
                <div class="col-sm-9 padding-right">
                    ...
                    <div class="features_items">
                            ...
                            <form id="form-sort" action="{{ url('category/' . $id) }}">
                                <div class="orderby-wrapper pull-left">
                                    ...
                                    <select name="sort" id="sort">
                                        ...
                                        <option value="az" @if (Request::get('sort') == 'az') selected @endif>Tên: A-Z
                                        </option>
                                        <option value="za" @if (Request::get('sort') == 'za') selected @endif>Tên: Z-A
                                        </option>
                                        <option value="asc" @if (Request::get('sort') == 'asc') selected @endif>Giá: Tăng
                                            dần</option>
                                        <option value="desc"@if (Request::get('sort') == 'desc') selected @endif>Giá: Giảm
                                            dần</option>
                                        <option value="new" @if (Request::get('sort') == 'new') selected @endif>Mới nhất
                                        </option>
                                        <option value="old" @if (Request::get('sort') == 'old') selected @endif>Cũ Nhất
                                        </option>
                                    </select>
                                </div>
                            </form>
                        ...
                        <div class="col-xs-6 nopadding-left">
                            <form class="tree-most" id="form-change-layout" method="get"
                                action="{{ url('category/' . $id) }}" method="get">
                                <div class="orderby-wrapper pull-right">
                                    ...
                                    <select id="display" name="display" class="display">
                                        <option value="grid" @if ($display == 'grid') selected @endif>Lưới
                                        </option>
                                        <option value="list" @if ($display == 'list') selected @endif>Danh sách
                                        </option>
                                    </select>
                                </div>
                            </form>
                                        ...
                                        <div class="product-image-wrapper item-product">
                                                ...
                                                <div class="productinfo text-center">
                                                    <form>
                                                        ...
                                                        <input type="hidden" value="{{ $pro->id }}"
                                                            class="cart_product_id_{{ $pro->id }}">
                                                        <input type="hidden" value="{{ $pro->name }}"
                                                            class="cart_product_name_{{ $pro->id }}">
                                                        <input type="hidden" value="{{ $pro->price }}"
                                                            class="cart_product_price_{{ $pro->id }}">
                                                        <input type="hidden" value="{{ asset($pro->getImagePrimary()) }}"
                                                            class="cart_product_image_{{ $pro->id }}">
                                                        <input type="hidden" value="1"
                                                            class="cart_product_qty_{{ $pro->id }}">
                                                        <a href="{{ URL::to('/product/' . $pro->id . '/detail') }}">
                                                            <img src="{{ asset($pro->getImagePrimary()) }}"
                                                                alt="" />
                                                            <div class="div-price">
                                            ...
                                            <div class="choose">
                                                <ul class="nav nav-pills nav-justified ">
                                                    <li id="li_{{ $pro->id }}"
                                                        @if (Auth::check() && $pro->checkProductWishList(Auth::id())) class="active-custom" @endif>
                                                        <a class="cursor" onclick="add_wishlist({{ $pro->id }})">
                                                            @if (!$pro->checkProductWishList(Auth::id()))
                                                                <i class="fa fa-plus-square"></i>Thêm yêu thích
                                                            @else
                                                                Đã yêu thích
                                                            @endif
                                                        </a>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    ...
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close"
                                                        style="margin-top: -26px; font-size: 30px; color: #FE980F">
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        ...
                                                        <form>
                                                            <div class="col-md-7">
                                                                <h2 class="product_quickview_title"><span
                                                                        id="product_quickview_title"></span></h2>
                                                ...
                                                <div class="modal-footer">
                                                    <a title="Thêm sản phẩm vào giỏ hàng" name="add_to_cart_quick_view"
                                                        type="button" class="btn btn-primary btn-custom">Mua ngay</a>
                                                    <a type="submit" href="{{ URL::to('cart') }}"
                                                        class="btn btn-primary btn-custom">Đi tới giỏ hàng</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="item item-custom">
                                            <div class="inner-item">
                                                <div class="item-image">
                                                    <div class="inner">
                                                        <a class="product-image no-touch"
                                                            href="{{ URL::to('/product/' . $pro->id . '/detail') }}">
                                                            <img class="first_image"
                                                                src="{{ asset($pro->getImagePrimary()) }}"
                                                                alt="" />
                                                        </a>
                                                    </div>
                                                </div>
                                                <div class="product-shop">
                                                    <div class="inner">
                                                        <h2 class="product-name"><a class="product-image"
                                                                href="{{ URL::to('/product/' . $pro->id . '/detail') }}"
                                                                title="">{{ $pro->name }}</a></h2>
                                                        <div class="price-box">
                                                            <span class="regular-price">
                                                                <h2 class="price">{{ number_format($pro->price) }}&#8363;
                                                                </h2>
                                                            </span>
                                                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="mt-3 paginate">
                                <center>{{ $products->withQueryString()->links() }}</center>
                            </div>
                        </div>
                    </div>
