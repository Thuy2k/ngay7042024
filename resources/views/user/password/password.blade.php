                            ...
                            <form id="form-checkout" action="{{ route('user.change_password.post') }}" method="POST">
                                ...
                                <div class="row">
                                    <div class="col-12 p-30">
                                        <center><a class="" href="{{ route('home') }}"><img width="100px"
                                                    src="{{ asset('user/images/home/logo.png') }}" alt="" /></a>
                                        </center>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="password" id="old_password" name="old_password"
                                        placeholder="Nhập mật khẩu cũ" class="form-control input-checkout">
                                    <div class="error error-old_password"
                                        @if ($errors->has('old_password')) style="display:block" @endif>
                                        {{ $errors->first('old_password') }}</div>
                                </div>
                                <div class="form-group">
                                    <label for="new_password">Mật khẩu mới </label>
                                    <input type="password" id="new_password" name="new_password"
                                        placeholder="Nhập mật khẩu mới" class="form-control input-checkout">
                                    <div class="error error-new_password"
                                        @if ($errors->has('new_password')) style="display:block" @endif>
                                        {{ $errors->first('new_password') }}</div>
                                </div>
                                <div class="form-group">
                                    <label for="confirm_password">Nhập lại mật khẩu </label>
                                    <input type="password" id="confirm_password" name="confirm_password"
                                        placeholder="Nhập lại mật khẩu" class="form-control input-checkout">
                                    <div class="error error-confirm_password"
                                        @if ($errors->has('confirm_password')) style="display:block" @endif>
                                ...
                                <div class="form-group">
                                    <div class="row">
                                        <div class="col-12">
                                            <center><button class="btn btn-custom" type="submit" id="btn-complete">Đổi mật
                                                    khẩu</button></center>
                                        </div>
                                    </div>
                                </div>
