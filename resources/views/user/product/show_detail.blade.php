...
@section('content')
    ...
    <input type="hidden" id="viewed_product_url{{ $product->id }}"
        value="{{ URL::to('/product/' . $product->id . '/detail') }}">
                                        ...
                                        <div>
                                            ...
                                            <div class="full-width pt-20-custom pb-20-custom">
                                                ...
                                                @if (count($colors) > 0)
                                                    @php
                                                        $color_white = '#ffffff';
                                                        [$rf, $gf, $bf] = sscanf($color_white, '#%02x%02x%02x');
                                                    @endphp
                                                    @foreach ($colors as $color)
                                                        ...
                                                        @endphp
                                                        <span
                                                            class="size-item item-color cursor @if (!empty($first_color) && $color->id == $first_color->id) active @endif"
                                                            style="background-color:{{ $color->color_code }}; @if (!$flag_color) color:white @else color:black @endif"
                                                            data-product="{{ $product->id }}"
                                                            data-color={{ $color->id }}>{{ $color->name }}</span>
                                                    @endforeach
                                                @endif
                                            </div>
                                                ...
                                                <div id="box-size">
                                                    @if (count($sizes) > 0)
                                                        @foreach ($sizes as $size)
                                                            <span
                                                                class="size-item item-size cursor @if (!empty($first_size) && $size->id == $first_size->id) active @endif size_{{ $size->id }}"
                                                                onclick="clickSize({{ $size->id }})">{{ $size->name }}</span>
                                                        @endforeach
                                                    @endif
                                                </div>
                                            </div>
                                            <span>
                                                <div class="price-display">
                                                    ...
                                                    <input id="color" type="hidden" name="color"
                                                        value="{{ !empty($first_color) ? $first_color->id : '' }}">
                                                    <input id="size" type="hidden" name="size"
                                                        value="{{ !empty($first_size) ? $first_size->id : '' }}">
                                                    <div class="full-width">
                                                        <button type="submit" id="add-cart"
                                                            class="btn btn-fefault cart btn-custom ml-custom">
                            ...
                            @if (count($tags) > 0)
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <ul class="box-tag">
                                            @foreach ($tags as $tag)
                                                <li class="box-tag-medium"><a
                                                        href="{{ route('user.tag.index', ['slug' => $tag->slug]) }}"
                                                        class="tag-product cursor">#{{ $tag->name }}</a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="reviews">
                                <div class="response-area">
                                        ...
                                        @if ((Auth::check() && $isBought) || $isAdmin)
                                            <li class="media">
                                                <div class="form-group pl-10-custom">
                                                    <textarea type="text" id="comment" rows="2" name="note" placeholder="Nhập bình luận"
                                                        class="form-control"></textarea>
                                                    <div class="error error-comment error-comment_text">Nhập nội dung bình
                                                        luận</div>
                                                    <button class="btn btn-custom" @click="comment()">Bình luận</button>
                                                </div>
                                            </li>
                                        @endif
                                        <li class="media" v-if="comments.length > 0"
                                            v-for="(comment, index) in comments">
                                            <a class="pull-left" href="#">
                                                <img class="media-object img-avatar" :src="getAvatar(comment.user.avatar)"
                                                    alt="">
                                            </a>
                                            <div class="media-body">
                                                ...
                                                @if ((Auth::check() && $isBought) || $isAdmin)
                                                    <span class="cursor title-reply" @click="showReply(comment.id)">Trả
                                                        lời</span>
                                                @endif
                                                @if (Auth::check() && $isBought)
                                                    <span v-if="checkPermission(comment.user_id)"
                                                        class="cursor title-reply pl-10px"
                                                        @click="deleteComment(comment.id)">Xoá</span>
                                                @endif
                                                @if ($isAdmin)
                                                    <span class="cursor title-reply pl-10px"
                                                        @click="deleteComment(comment.id)">Xoá</span>
                                                @endif
                                                @if ((Auth::check() && $isBought) || $isAdmin)
                                                    <ul class="form-child d-none" :id="getClassForm(comment.id)">
                                                        <li class="media">
                                                            <div class="form-group pl-10-custom">
                                                                <textarea type="text" :class="getNameReply(comment.id)" rows="2" name="note"
                                                                    placeholder="Nhập bình luận" class="form-control"></textarea>
                                                                <div class="error error-comment"
                                                                    :class="getErrorName(comment.id)">Nhập nội dung bình
                                                                    luận</div>
                                                                <button class="btn btn-custom"
                                                                    @click="replyComment(comment.id)">Bình luận</button>
                                                            </div>
                                                        </li>
                                                    </ul>
                                                @endif
                                                ...
                                                <li class="media second-media" v-for="(item, index) in comment.children">
                                                    <a class="pull-left" href="#">
                                                        <img class="media-object img-avatar"
                                                            :src="getAvatar(item.user.avatar)" alt="">
                                                    </a>
                                                    <div class="media-body">
                                                        <ul class="sinlge-post-meta">
                                                        ...
                                                        @if (Auth::check() && $isBought)
                                                            <span v-if="checkPermission(item.user_id)"
                                                                class="cursor title-reply pl-10px"
                                                                @click="deleteComment(item.id,item.parent_id)">Xoá</span>
                                                        @endif
                                                        @if ($isAdmin)
                                                            <span class="cursor title-reply pl-10px"
                                                                @click="deleteComment(item.id,item.parent_id)">Xoá</span>
                                    ...
                                    @if (count($comments) >= 5)
                                        <div class="form-group pl-10-custom">
                                            <center><button class="btn btn-custom" v-if="!all" @click="showMore()">Xem
                                                    thêm</button></center>
                                        </div>
                                    @endif
                                </div><!--/Response-area-->
                    ...
                    if (!res.error) {
                        if (res.data.length > 0) {
                            $.each(res.data, function(key, value) {
                                html += '<span class="size-item item-size cursor size_' + value
                                    .id + '" onclick="clickSize(' + value.id + ')" >' + value
                                    .name + '</span>';
                            });
                        }
                        $('#box-size').html(html);
