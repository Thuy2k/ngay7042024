    ...
    <div class="left-sidebar">
        ...
        <div class="panel-group category-products scroll-menu" id="accordian"><!--category-productsr-->
            @foreach ($categories as $key => $cate)
                @if ($cate->parent_id == 0)
                    <div class="panel panel-default cat-product-sb">
            ...
            <div class="price-range">
                ...
                <div class="well text-center">
                    <form id="form-filter" method="get" action="{{ url('/home') }}">
                        <div class="row">
                            <input type="text" name="range_price" class="span2" value=""
                                data-slider-min="{{ $min_price }}" data-slider-max="{{ $max_price }}"
                                data-slider-step="5"
                                data-slider-value="[{{ $min ?? $min_price }},{{ $max ?? $max_price }}]"
                                id="sl2"><br />
                            <b class="pull-left">{{ format_price($min_price) }}&#8363;</b> <b
                                class="pull-right">{{ format_price($max_price) }}&#8363;</b>
                        </div>
                        <div class="row ">
                            <input type="submit" value="Lọc giá" id="btn-filter"
                                class="btn btn-sm btn-default btn-custom">
                        </div>
                    </form>
                </div>
            </div>
